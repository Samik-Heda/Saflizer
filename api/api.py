            ...
            if now.minute == supervisortime.minute and now.hour == supervisortime.hour:
                for driver in data[supervisor]['drivers'].keys():
                    ...
                    if lasttest == "None":
                        ...
                        if ((datetime.strptime(lasttest,
                                               '%d-%m-%Y %H:%M') - supervisortime).total_seconds() / 60) < 31 and ((
                                                                                                                           datetime.strptime(
                                                                                                                                   lasttest,
                                                                                                                                   '%d-%m-%Y %H:%M') - supervisortime).total_seconds() / 60) > 0 and \
                                data[supervisor]['drivers'][driver]['duty'] == 'True':
                            pass
                        else:
                            alertf(data, supervisor, driver, 'Test not completed')
                            datetimeinfo = datetime.now(tz=ZoneInfo('Asia/Kolkata')).strftime('%d-%m-%Y %H:%M')
...
def adddriver():
    ...
    if data['apikey'] == key:
        ...
        if data[username]['drivers'] != 'None':
            data[username]['drivers'][driverid] = {'name': name, 'image': image, 'duty': True, 'test': ['empty'],
                                                   "lasttest": "None"}
        else:
            data[username]['drivers'] = {
                driverid: {'name': name, 'image': image, 'duty': True, 'test': ['empty'], "lasttest": "None"}}
        uploaddataf(data)
        return "200"
    else:
        return "401"
